...
func TestValidateCert(t *testing.T) {
	...
	if ok := validCert.Validate(time.Now()); !ok {
	...
	if ok := invalidCert.Validate(time.Now()); ok {
...
func TestValidateCertTime(t *testing.T) {
	...
	for ts, ok := range map[int64]bool{
		...
		if v := cert.Validate(time.Unix(ts, 0)); v != ok {
			t.Errorf("Validate(%d): %v", ts, v)
		}
	}
}
