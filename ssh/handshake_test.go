...
func (t *testChecker) Check(dialAddr string, addr net.Addr, alg string, key []byte) error {
	if dialAddr == "bad" {
...
func TestHandshakeBasic(t *testing.T) {
	trC, trS, err := handshakePair(&ClientConfig{}, "addr")
	if err != nil {
	...
	if trC.conn.(*transport).reader.cipherAlgo == "none" {
		t.Errorf("transport did not change algorithm.")
	}
}

func TestHandshakeError(t *testing.T) {
	trC, trS, err := handshakePair(&ClientConfig{}, "bad")
	if err != nil {
		t.Fatalf("handshakePair: %v", err)
	}
	defer trC.Close()
